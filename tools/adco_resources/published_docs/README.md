...
Local-first, Codex-first control plane for advertising creative operations.

It turns messy briefs, references, image assets, specialist outputs, PPT drafts, review gates, and client-visible risk into an exact-current, traceable project folder.

## Current Release

The current public release is [`v0.3.2`](https://github.com/papperrollinggery/ad-creative-orchestrator/releases/tag/v0.3.2). The Release tag is the authoritative source commit and wheel provenance.

Compared with the earlier monolithic workflow, the current release adds focused Skill references, Human Workspace v2 current-first indexes, schema-v2 migration and legacy quarantine, fail-closed FinalDelivery reconciliation, immutable Thread dispatch proof, and exact artifact/hash binding for film-specialist adoption. `VALIDATION=PASS` still means structural and traceability validation only; it never means client approval or creative-quality acceptance.

`v0.3.2` also packages and parity-checks the public README, changelog, operating guides, and first-run assets. As a result, `adco docs` exposes real documentation paths in both source-checkout and installed-wheel modes.

Documentation status:

- `README.md` and `docs/operating/` are the current user/operator contract.
- `docs/design/` records design rationale; files explicitly marked archived may contain retired conceptual route names and must not be used as CLI instructions.
- `docs/reviews/` contains historical review receipts. Dates, paths, and commands there are evidence from the recorded run, not current installation guidance.
- `docs/context/` is background context; current commands and release claims still come from this README and `docs/operating/`.

## Quickstart

Install the current GitHub Release with `pipx`:
...
- Human Workspace v2: exact-current-first indexes, canonical path deduplication, physical-file discovery, and explicit missing/unregistered evidence.
- Schema-v2 lifecycle controls: legacy quarantine, traceable tombstones, strict legacy validation, and fail-closed FinalDelivery reconciliation.
...
- `adco docs`: prints README, changelog, operating-guide, release-plan, and first-run-document paths in both source and installed modes.
...
常用命令（完整参数以 `adco --help` 和各子命令 `--help` 为准）：
...
adco final-delivery-reconcile <项目目录> --old-path <旧路径> --new-path <新路径> --kind rename|supersession --confirmed-by <人工确认者> --confirmed-at <iso_time> --evidence-ref <结构化确认回执.json> [--version-id <版本ID>]
...
adco thread-observe <项目目录> --work-id <WORK-ID> --lane-id <LANE-ID> --state <状态> --observed-at <iso_time> --evidence <证据>
adco thread-reconcile <项目目录> --work-id <WORK-ID> --lane-id <LANE-ID> --receipt-path <回执> --adoption-decision ADOPT|PARTIAL_ADOPT|REJECT|BLOCKED --reconciled-at <iso_time> --cleanup-action <动作>
...
```text
先运行 adco docs，并读取输出中的 SKILL_DRAFT
读取项目根目录 AGENTS.md 后，先执行只读 status 路线；需要 CLI 时运行 adco status <项目目录>
```

`start/status`、`add materials`、`next`、`gate/review` 等名称是 Agent 级语义路线，不是额外 CLI 子命令；命令行始终使用 `adco status`、`adco run --material`、`adco next` 和对应阶段 Gate。
...
5. 让 Codex 读取项目规则后执行 `adco next <项目目录>`
```
...
发布必须以 `v0.3.2` GitHub Release 的 tag target、Actions 运行、Release wheel 以及全局重装后的来源/hash 回读为准；版本号相同不能代替产物证据。
...
docs diagnostics: source checkout and installed wheel both report packaged public docs with every listed path present
