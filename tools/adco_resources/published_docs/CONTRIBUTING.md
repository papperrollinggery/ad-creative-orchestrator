...
The release gate covers syntax, goal/specialist/lifecycle regression, template and Skill parity, published-document parity, both example projects, dashboard audit, wheel inspection, editable install smoke, and normal package install smoke.
...
- Update README, CHANGELOG, operating docs, packaged published docs, Skill references, and package metadata together when behavior or positioning changes.
- Do not use retired colon-prefixed pseudo-command labels in current onboarding docs; use the real `adco` CLI and identify Agent-level routes as semantic routes.
